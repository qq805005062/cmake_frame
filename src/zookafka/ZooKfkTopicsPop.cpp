...
int ZooKfkTopicsPop::zookInit(const std::string& zookeepers, const std::string& topic)
{
	int ret = 0;
	char brokers[1024] = {0};
...
int ZooKfkTopicsPop::kfkInit(const std::string& brokers, const std::string& topic)
{
...
int ZooKfkTopicsPop::pop(std::string& topic, std::string& data, int64_t* offset, std::string* key)
{
	...
	if(top)
	{
		PDEBUG("len: %zu,topic :: %s partition: %d, offset: %ld\n", message->len, top, message->partition, message->offset);
	}else{
