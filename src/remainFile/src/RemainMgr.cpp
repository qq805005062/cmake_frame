...
int RemainMgr::writeRemainFile(int bitFlag, const std::string& userId, const char *data, size_t dataSize)
{
	int ret = 0;
	if(isInit_ == 0)
	{
	...
	WriteFileMgrPtr fileMgrP(nullptr);
	{
		...
		UserIdFileMgrMapIter iter = userFileMgrMap_.find(userId);
		if(iter == userFileMgrMap_.end())
		{
			fileMgrP.reset(new WriteFileMgr(gateNo_, maxCount_, outSecond_, sharePath_, localPath_, userId));
			if(fileMgrP == nullptr)
			{
				WARN("RemainMgr writeRemainFile WriteFileMgrPtr new nullptr");
				return -132;
			}
			userFileMgrMap_.insert(UserIdFileMgrMap::value_type(userId, fileMgrP));
		}else{
...
int RemainMgr::writeRemainFileBak(int bitFlag, const std::string& userId, const char *data, size_t dataSize)
{
	int ret = 0;
	if(isInit_ == 0)
	{
...
int RemainMgr::readRemainFile(int bitFlag, const std::string& userId, std::string& remainData)
{
	int ret = 0;
	if(isInit_ == 0)
	{
	...
	switch(bitFlag)
	{
		case REMAIN_MGR_RE_FLAG:
			{
				ReadDirMgrPtr dirRead(new ReadDirMgr(RE_REMAIN_FLAG, gateNo_, userId, sharePath_, localPath_));
				if(dirRead)
				{
		...
		case REMAIN_MGR_MO_FLAG:
			{
				ReadDirMgrPtr dirRead(new ReadDirMgr(MO_REMAIN_FLAG, gateNo_, userId, sharePath_, localPath_));
				if(dirRead)
				{
					ret = dirRead->readReaminData(remainData);
					INFO("dirRead->readReaminData ret %d", ret);
		...
		case REMAIN_MGR_MT_FLAG:
			{
				ReadDirMgrPtr dirRead(new ReadDirMgr(MT_REMAIN_FLAG, gateNo_, userId, sharePath_, localPath_));
				if(dirRead)
				{
					ret = dirRead->readReaminData(remainData);
					INFO("dirRead->readReaminData ret %d", ret);
...
int RemainMgr::statisticsRemainCount(int bitFlag, const std::string& userId, int& localCount, int& shareCount)
{
	if(isInit_ == 0)
	{
