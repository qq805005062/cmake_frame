...
ReadDirMgr::ReadDirMgr(int flag, int gateNo, const std::string& userId, const std::string& sharePath, const std::string& localPath)
	:bitFlag_(flag)
	,gateNo_(gateNo)
	,userName_(userId)
	,shareRootPath_()
	...
	,localPath_()
{
	...
	if(flag == RE_REMAIN_FLAG)
	{
		if(!sharePath.empty())
		{
			...
			sprintf(shareTmpPath, "%s/%s/%s", sharePath.c_str(), reportDir.c_str(), userId.c_str());
		}
		sprintf(localTmpPath, "%s/%s/%s", localPath.c_str(), reportDir.c_str(), userId.c_str());
	...
	}else if(flag == MO_REMAIN_FLAG)
	{
		...
		{
			shareRootPath_.assign(sharePath);
			sprintf(shareTmpPath, "%s/%s/%s", sharePath.c_str(), moDir.c_str(), userId.c_str());
		}
		sprintf(localTmpPath, "%s/%s/%s", localPath.c_str(), moDir.c_str(), userId.c_str());
		sharePath_.assign(shareTmpPath);
		localPath_.assign(localTmpPath);
	}else if(flag == MT_REMAIN_FLAG)
	{
		...
		{
			shareRootPath_.assign(sharePath);
			sprintf(shareTmpPath, "%s/%s/%s", sharePath.c_str(), mtDir.c_str(), userId.c_str());
		}
		sprintf(localTmpPath, "%s/%s/%s", localPath.c_str(), mtDir.c_str(), userId.c_str());
		sharePath_.assign(shareTmpPath);
		localPath_.assign(localTmpPath);
	}else{
		ERROR("ReadDirMgr flag %d", flag);
	}
	DEBUG("ReadDirMgr init bitFlag_ %d gateNo_ %d userName_ %s sharePath_ %s localPath_ %s", bitFlag_, gateNo_, userName_.c_str(), sharePath_.c_str(), localPath_.c_str());
}
