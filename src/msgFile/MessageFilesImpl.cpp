...
bool MessageFilesImpl::InitParams(const std::string& szPtCode, int gateNo)
{
...
void MessageFilesImpl::updateGenFreq(int freq)
{
    freq < 1 ? freq = 60 : 1;
    freq > 1800 ? freq = 60 : 1;
    ...
    for (auto it = details_.begin(); it != details_.end(); ++it)
    {
        if (it->second)
        {
            it->second->updateGenFreq(freq);
        }
    }
}

void MessageFilesImpl::updateMaxFileSize(int size)
{
    size < 1 ? size = 20 : 1;
    size > 1000 ? size = 20 : 1;
    ...
    for (auto it = details_.begin(); it != details_.end(); ++it)
    {
        if (it->second)
        {
            it->second->updateMaxFileSize(size);
        }
    }
}

int MessageFilesImpl::InitMtDetailsFile(const std::string& szPtCode,
        int gateNo,
...
int MessageFilesImpl::InitMtOkDetailsFile(const std::string& szPtCode,
        int gateNo,
        const std::string& fileType,
        int maxFileSize,
        int genFreq)
...
int MessageFilesImpl::InitReDetailsFile(const std::string& szPtCode,
    int gateNo,
    const std::string& fileType,
    int maxFileSize,
    int genFreq)
...
int MessageFilesImpl::InitReOkDetailsFile(const std::string& szPtCode,
    int gateNo,
    const std::string& fileType,
    int maxFileSize,
    int genFreq)
...
int MessageFilesImpl::InitMoDetailsFile(const std::string& szPtCode,
    int gateNo,
    const std::string& fileType,
    int maxFileSize,
    int genFreq)
...
int MessageFilesImpl::InitMoOkDetailsFile(const std::string& szPtCode,
    int gateNo,
    const std::string& fileType,
    int maxFileSize,
    int genFreq)
...
{
    FileMgrMap::iterator iter = details_.end();
    {
        std::lock_guard<std::mutex> lock(mutex_);
        iter = details_.find(fileType);
    ...
    if (iter != details_.end())
    {
        if (iter->second)
        {
            return iter->second->WriteToFile(data.c_str(), data.length());
        }
    }
    return -2;
}
