...
int Daemon::DaemonStart(const char* pidfile) 
{
    ...
    if (getpid() == 1) 
    {
        fprintf(stderr, "already daemon, can't start, getppid(): %d\n", getppid());
        return -1;
    }
...
void Daemon::ChangeTo(const char* argv[]) 
{
    ...
    { //child
        ...
        execvp(argv[0], reinterpret_cast<char* const* >(const_cast<char** >(argv)));
    }
}
...
namespace 
{
    map<int, function<void(int)>> handlers;
...
void Signal::signal(int sig, const function<void(int)>& handler) {
